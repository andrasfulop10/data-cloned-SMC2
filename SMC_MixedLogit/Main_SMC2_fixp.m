%data-cloning smc2
%fixed p (number of state particles)
...
%parameter setup
N_latent_particles=200;  %initial number of latent particles p
...
%setting number of latent particles (p)
...
%acceptance rate below which number of state particles is doubled. given
%that here it is set to 0, and teh acceptance rate cannot be negative, it is equivalent to
%fixing p. 
...
for isim=1:Nrep
    ...
    %remember to change the workspace name to avoid overwrite
    save result_mlogit_fixp_p200 dataX dataY Nparam Xsample smcsettings filtersettings output;   
