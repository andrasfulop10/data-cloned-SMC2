function [X_proposal,lnprop_X,lnprop_Xprop]=f_SimProposalTrMixtureNormal(X,expindex,logisticindex,...
...
if ~isempty(logisticindex)
    ...
    X_tr(:,logisticindex)=f_mylogisticinv(X_tr(:,logisticindex),logisticbound_l,logisticbound_u);
...
logJacobian_prop=logJacobian_prop+...
    sum(-log(f_mylogisticder(X_tr_proposal(:,logisticindex),logisticbound_l,logisticbound_u)),2);


logJacobian=logJacobian+...
    sum(-log(f_mylogisticder(X_tr(:,logisticindex),logisticbound_l,logisticbound_u)),2);
...
if ~isempty(logisticindex)
    ...
    X_proposal(:,logisticindex)=f_mylogistic(X_tr_proposal(:,logisticindex),logisticbound_l,logisticbound_u);
